...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager.create_table(
            Table::create()
                ...
                .col(timestampz_default(LoginSession::CreatedAt))
                .col(timestampz(LoginSession::ValidTill))
                .foreign_key(
                    ForeignKey::create()
                        .name("fk-post_user")
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager.drop_table(Table::drop().table(LoginSession::Table).to_owned()).await
    }
}
